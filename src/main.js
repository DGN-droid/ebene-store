...
import { changeQuantity, getCart, removeFromCart } from './cart.js';
...
const card = (product) => `<article class="product-card" data-product-card><a class="product-card__image" href="produit.html?id=${product.id}"><img src="${product.image}" alt="${product.name}"></a><div class="product-card__body"><p>${product.category}</p><h3>${product.name}</h3><p class="price">${currency(product.price)}</p><a class="button" href="produit.html?id=${product.id}">Voir le produit</a></div></article>`;
...
async function renderProducts() { const target = document.querySelector('[data-products]'); if (!target) return; target.innerHTML = (await loadProducts()).map(card).join(''); }
async function renderProductPage() { const target = document.querySelector('[data-product-detail]'); if (!target) return; const products = await loadProducts(); const id = new URLSearchParams(location.search).get('id') || '1'; const product = products.find((item) => item.id === id) || products[0]; const robeBackdrop = product.id === '2' ? '<video class="product-detail-background" data-lazy-video autoplay loop muted playsinline aria-hidden="true"><source src="assets/videos/robe3D.mp4" type="video/mp4"></video><div class="product-detail-video-scrim" aria-hidden="true"></div>' : '';
  target.innerHTML = `${robeBackdrop}<div class="product-detail-content"><div class="media-frame"><img class="product-detail-image" src="${product.image}" alt="${product.name}"></div><div><p>${product.category}</p><h1>${product.name}</h1><p class="price">${currency(product.price)}</p><p>${product.description}</p><div class="quantity-control"><button data-detail-change="-1" aria-label="Diminuer"><i data-lucide="minus"></i></button><span data-detail-quantity>0</span><button data-detail-change="1" aria-label="Augmenter"><i data-lucide="plus"></i></button></div> <button class="button button--accent" data-add-product="${product.id}">Ajouter au panier</button></div></div>`; const refresh = () => { document.querySelector('[data-detail-quantity]').textContent = getCart().find((item) => item.id === product.id)?.quantity || 0; }; target.addEventListener('click', (event) => { const amount = event.target.closest('[data-detail-change]')?.dataset.detailChange; if (amount) { changeQuantity(product.id, Number(amount)); refresh(); } if (event.target.closest('[data-add-product]')) { changeQuantity(product.id, 1); refresh(); } }); refresh(); renderIcons(); initVideoObserver(); }
...
function initThemeMenu() {
  ...
  refresh();
}document.addEventListener('DOMContentLoaded', async () => { initTheme(); try { await loadPartial('#header-slot', '/src/partials/header.html'); initHeaderInteractions(); initThemeMenu(); const language = localStorage.getItem('ebene-language') || 'fr'; await setLanguage(language); document.querySelector('[data-language]').value = language; document.querySelector('[data-language]').addEventListener('change', (event) => setLanguage(event.target.value)); document.addEventListener('cartchange', () => renderCart()); initSearch(); initCarousels(); initVideoObserver(); renderIcons(); await renderProducts(); await renderProductPage(); await renderCart(); } finally { document.body.classList.remove('is-loading'); document.body.classList.add('is-ready'); } });
